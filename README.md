...
- 从历史报告入口重新打开成功报告，并继续查看处理中、失败或被拒绝的任务。
- 保留完整阶段对齐证据，并用独立的源时间戳显示时钟播放、暂停、拖动和跳转六个姿态事件代理。
- 三种报告模式默认使用同一组 17 个核心关节；完整 MediaPipe 33 点保留在数据中，仅在桌面调试开关启用后显示。
- 三种模式共享一个经过时间主时钟；播放时只对展示副本做帧间插值和置信度约束平滑，暂停、事件跳转和导出仍使用原始分析数据。
- 骨架叠加和动作通道按整段动作计算同一固定取景，完整保留头、手、脚与通道描边，播放时不会逐帧呼吸缩放。
- 骨架叠加不绘制含义不明确的跨骨架连线；差异继续通过身体区域高亮和可追溯数值证据表达。
- 报告按已有视角、全身覆盖、姿态与相机证据标记“基础可叠加 / 仅供参考 / 仅并排可用”；不可靠时关闭叠加和通道，但保留并排视频。
