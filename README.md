...
- 用户视频拒绝脚部出框、非侧面、相机明显移动、事件不完整等低可信输入；Demo 阶段模板只把画质和角度问题记为警告。
...
- 用户视频需要单人、固定机位、正常速度和完整投篮动作。
- 参考模板允许慢放或剪辑变速；此类模板只参与姿态和动作阶段比较，时间戳速度特征自动关闭。
- 竖屏或横屏均可，但必须持续看见头、髋、膝、踝和双脚。
- 使用投篮手一侧的侧面视角；模板和用户投篮手必须一致。
- 用户视频不要剪辑变速；所有视频都不能倒放、镜像、明显平移/缩放或让其他人物进入主体区域。

用户输入不可信或视频没有足够姿态数据生成六事件时，任务进入 `rejected` 并给出原因，不会伪造报告；Demo 阶段模板的时长、覆盖、角度和相机检查为软门禁。系统异常才进入可重试的 `failed`。
