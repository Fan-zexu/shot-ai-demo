...
- 每个阶段独立执行 15% Sakoe–Chiba band 的受限 DTW，单个源帧最多连续映射 4 个输出帧。
- 正常速度输入的 DTW 特征权重为角度 `0.50`、重定向位置 `0.30`、归一化速度 `0.20`；参考模板包含慢放或剪辑变速时，速度权重强制为 `0`，只使用姿态与阶段证据。缺失特征不补零，剩余权重重新归一。
