...
Demo 阶段的参考模板和用户视频都可以包含慢放或剪辑变速：Worker 仍检查可解码性和单调时间戳，但质量阈值失败只写入 `warning`；产物将 `normalSpeedConfirmed` 记录为 `false`，供比较引擎关闭时间戳速度特征。无法解码、没有姿态帧、主体轨迹确实歧义或无法形成六个有序事件仍会拒绝。
